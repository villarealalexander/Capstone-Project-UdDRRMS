...
@section('top-nav-links')
    <form id="searchForm" action="{{ route('Archiver.index') }}" method="GET" class="flex mx-2 items-center border border-white rounded-full overflow-hidden shadow-md">
    ...
    <a href="{{ route('Archiver.archives') }}" class="hover:bg-blue-600 px-2 text-white py-1 rounded-lg font-semibold text-md mx-2">
    ...
    <script>
        ...
        function submitForm() {
            ...
            const url = "{{ route('Archiver.index') }}?query=" + query;
...
@section('content')
    <div class="w-full items-center mx-auto mt-4 md:w-[650px] lg:w-full">
        <form id="deleteForm" action="{{ route('Archiver.confirm-student-delete') }}" method="GET">
            @csrf
                    ...
                    <thead class="bg-white sticky top-0">
                        <tr>
                            ...
                            <th class="px-2 py-2 border-gray-500 border-b-2">
                                <a href="{{ route('Archiver.index', ['sort_field' => 'month_uploaded', 'sort_direction' => ($sortParams['field'] === 'month_uploaded' && $sortParams['direction'] === 'asc') ? 'desc' : 'asc']) }}">
                        ...
                        @foreach($students as $student)
                            <tr>
                                ...
                                <a href="#"  class="text-white hover:bg-orange-500 bg-orange-300 rounded-md px-2"
                                    onclick="openChecklistModal('{{ $student->name }}', '{{ route('Archiver.checklist', ['student_id' => $student->id]) }}')">
                                        Checklist
                                </a>
                            </td>
                            </tr>
    ...
    @include('Archiver.upload')
    @include('Archiver.confirm-student-delete')
