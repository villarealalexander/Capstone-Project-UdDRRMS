        ...
        <div class="bg-white p-6 rounded-lg shadow-lg w-11/12 sm:w-3/4 md:w-1/2 lg:w-1/3">
            ...
            <form action="{{ route('MIS.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="password" name="MIS_password" id="MIS_password" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                    @if ($errors->has('MIS_password'))
                        <p class="text-red-500 text-xs italic mt-2">{{ $errors->first('MIS_password') }}</p>
                    @endif
                </div>
