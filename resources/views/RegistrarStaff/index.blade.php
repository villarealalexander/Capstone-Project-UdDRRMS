...
@section('title', 'RegistrarStaff Page')

@section('top-nav-links')
    <form id="searchForm" action="{{ route('RegistrarStaff.index') }}" method="GET" class="flex mx-2 items-center border border-white rounded-full overflow-hidden shadow-md">
    ...
    <script>
        ...
        function submitForm() {
            ...
            const url = "{{ route('RegistrarStaff.index') }}?query=" + query;
                    ...
                    <thead class="bg-gray-200 sticky top-0">
                        <tr>
                            ...
                            <th class="px-2 py-2 border-gray-500 border-b-2">
                                <a href="{{ route('RegistrarStaff.index', ['sort_field' => 'month_uploaded', 'sort_direction' => ($sortParams['field'] === 'month_uploaded' && $sortParams['direction'] === 'asc') ? 'desc' : 'asc']) }}">
                        ...
                        @foreach($students as $student)
                            <tr>
                                ...
                                <a href="#"  class="text-white hover:bg-orange-500 bg-orange-300 rounded-md px-2"
                                    onclick="openChecklistModal('{{ $student->name }}', '{{ route('RegistrarStaff.checklist', ['student_id' => $student->id]) }}')">
                                        Checklist
                                </a>
                            </td>
                            </tr>
